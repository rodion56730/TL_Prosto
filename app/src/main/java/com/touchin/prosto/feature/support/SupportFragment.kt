...
class SupportFragment :
    BaseContentFragment<SupportState,
            SupportViewModel, SupportController>(R.layout.fragment_support) {
    ...
    override fun render(state: SupportState, controller: SupportController) {
        ...
        binding.subjectInput.showError(state.hasSubjectError, R.string.common_email_text_error)
        binding.bodyInput.showError(state.hasBodyError, R.string.common_email_text_error)
    }
